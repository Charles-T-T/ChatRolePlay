...
def response_postprocess(text, dialogue_bra_token="「", dialogue_ket_token="」"):
    ...
    for line in lines:
        line = line.strip(" ")
        match = re.match(
            r"^(.*?)[:：]" + dialogue_bra_token + r"(.*?)" + dialogue_ket_token + r"$",
            line,
        )
...
def download_models():
    ...
    model = AutoModel.from_pretrained(
        "silk-road/luotuo-bert-medium", trust_remote_code=True, model_args=model_args
    ).to(device)
...
def luotuo_en_embedding(texts):
    ...
    if _luotuo_model_en is None:
        ...
        _luotuo_model_en = AutoModel.from_pretrained("silk-road/luotuo-bert-en").to(
            device
        )
    ...
    inputs = _luotuo_en_tokenizer(
        texts, padding=True, truncation=False, return_tensors="pt"
    )
    inputs = inputs.to(device)

    with torch.no_grad():
        embeddings = _luotuo_model_en(
            **inputs, output_hidden_states=True, return_dict=True, sent_emb=True
        ).pooler_output
...
def get_embedding_for_chinese(model, texts):
    ...
    for i in range(len(texts)):
        if len(texts[i]) > 510:  # NOTE 为啥截510？
            texts[i] = texts[i][:510]
    if len(texts) >= 64:
        embeddings = []
        chunk_size = 64
...
def luotuo_openai_embedding(texts, is_chinese=None):
    """
    根据输入文本的语言（中文或英文）选择不同的嵌入方法，支持批量处理和单条处理。

    Args:
        texts: 输入文本，可以是字符串或字符串列表。
        is_chinese: 可选参数，明确指定语言类型。
            - True: 视为中文
            - False: 视为英文
            - None: 自动检测语言

    Returns:
        嵌入向量，单条输入返回单个嵌入，列表输入返回嵌入列表。
    """
    # 获取 OpenAI API 密钥
    openai_key = os.environ.get("OPENAI_API_KEY")

    # 初始化中文模型
    luotuo_model = get_luotuo_model()
    ...
    def process_single(text):
        ...
        try:
            ...
            if is_chinese is None:
                lang = is_chinese_or_english(text)
            else:
                lang = "chinese" if is_chinese else "english"

            if openai_key is None or lang == "chinese":
                return get_embedding_for_chinese(luotuo_model, text)[0].cpu().tolist()
            else:
                return get_embedding_for_english(text)
