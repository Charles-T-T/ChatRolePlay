...
class ChatRolePlay:
    ...
    def __init__(
        ...
        super(ChatRolePlay, self).__init__()
        ...
        if embedding == "luotuo_openai":
            ...
            print(
                f"warning! undefined embedding {embedding}, use luotuo_openai instead."
            )
            self.embedding = luotuo_openai_embedding
        ...
        if role_from_hf:
            ...
            if embedding == "luotuo_openai":
                ...
                print(
                    "warning! unkown embedding name ", embedding, " while loading role"
                )
                embed_name = "luotuo_openai"

            texts, vecs, self.system_prompt = self.extract_text_vec_from_datas(
                datas, embed_name
            )
            ...
            if self.db_type == None:
                ...
                print(
                    "warning! directly load folder from dbtype ",
                    self.db_type,
                    " has not been implemented yet, change back to chroma,or try use role_from_hf to load role instead",
                )
                self.db_type = "chorma"
            self.db = get_db_from_type(self.db_type)
    ...
    def get_models(self, model_name):
        ...
        if model_name == "openai":
            ...
            qwen_models = qwen_api.list_models(
                filter=ModelFilter(model_name=model_name), author="silk-road"
            )
            qwen_models_id = []
    ...
    # TODO 将故事背景长度/对话历史长度改为全局变量，并尝试找个更好的参数
    ...
    def build_story_db_from_vec(self, texts, vecs):
        self.db = get_db_from_type(self.db_type)
        self.db.init_from_docs(vectors=vecs, documents=texts)
    ...
    def chat(self, role, text):
        ...
        response = response_postprocess(
            response_raw, self.dialogue_bra_token, self.dialogue_ket_token
        )
    ...
    def add_story(self, query):
        ...
        dialogue_divide_token_length = self.tokenizer(self.dialogue_divide_token)

        for story in stories:
            story_token = self.tokenizer(story)
            total_story_token = story_token + dialogue_divide_token_length
            if sum_story_token + total_story_token > self.max_len_story:
                break
            sum_story_token += total_story_token
    ...
    def add_history(self):
        if not self.dialogue_history:  # 如果历史为空，直接返回
            return

        sum_history_token = 0
        history_to_add = []  # 用于存储符合长度限制的历史记录
        ...
        for query, response in reversed(self.dialogue_history):
            ...
            # 检查是否超出最大长度限制
            if sum_history_token + current_count > self.max_len_history:
                break
            sum_history_token += current_count
            history_to_add.append((query, response))

        # 如果没有历史记录可以添加，打印警告
        if not history_to_add:
            print("warning! no history added. the last dialogue is too long.")
            return

        # 按时间顺序将符合限制的历史记录添加到上下文
        for query, response in reversed(history_to_add):
            if query:
                self.llm.user_message(query)
            if response:
